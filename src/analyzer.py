...
3c. 业绩弹性测算（分业务弹性树、价格敏感度公式、悲观/基准/乐观三情景分析）
3d. 风险分析（风险清单表格、逻辑破坏条件与止损信号）
3e. 质量评分（100分制，5个维度：基本面质量30分、产业匹配度20分、业绩弹性20分、估值与位置15分、资金与治理15分）
...
"""
    },
    {
        ...
        "prompt": """
必须包含：
4a. 估值方法选择（成长型用PE/PEG、周期型用PB/EV-EBITDA、困境反转用PB+减值出清）
4b. 资金面分析（表格：筹码分布、资金流向、关键判断）
4c. 技术面分析（表格：价格位置、均线系统、成交量、技术指标、关键价格位）
4d. 三档目标价（表格：短期/中期/长期，包含时间窗口、目标价、估值依据、核心假设、触发条件、盈亏比）
4e. 盈亏比量化（当前位置：向下空间XX%、向上空间XX%、盈亏比X:X）
4f. 对标分析（本票 vs 对标公司A vs 对标公司B，多维度对比）
...
"""
    },
    {
        ...
        "prompt": """
必须包含：
5a. 分层跟踪锚点（表格：高频周度、季度、事件驱动，包含跟踪内容和数据来源）
5b. 执行清单（短线：触发条件、失效条件；中线：3财务+3事件）
5c. 综合结论（强制格式）：
...
"""
    ...
    output_file = f"output/zbs-{stock_name}.md"
...
def analyze_stock(stock_code, data_file):
    ...
    ⚠️ 重要：这个函数只是框架，实际的Part I-V分析需要AI执行

    AI执行时必须：
    1. 读取data_file获取akshare数据
    2. 执行MCP搜索获取行业、竞争对手、研报数据
    3. 严格按照ANALYSIS_STEPS配置，逐步完成Part I-V
    4. 每完成一个Part，输出：✅ Part X完成（XX行）
