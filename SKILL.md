---
name: zbs-stock-research
description: ZBS智能选股+个股深度研究系统，提供6种选股策略和三层架构分析（数据采集+AI分析+HTML生成），基于AkShare免费数据。当用户说"分析XXX股票"或"选股/推荐股票"时触发。
trigger_keywords: ["分析股票", "个股分析", "股票研究", "基本面分析", "选股", "推荐股票", "选股策略"]
version: 3.0
...
> 版本：v3.0 | 更新：2026-05-19 | 定位：选股推荐 + 三层架构分析 + AI手写HTML
...
**当用户说"选股"、"推荐股票"时，使用选股系统：**

```bash
cd D:/AAAkaifa/claude/股票分析

# 单策略选股
python stock_screener.py --strategy oversold --top 10    # 超跌反弹
python stock_screener.py --strategy trend --top 10       # 趋势跟踪
python stock_screener.py --strategy dividend --top 10    # 高股息价值
python stock_screener.py --strategy growth --top 10      # 成长股
python stock_screener.py --strategy northbound --top 10  # 北向资金跟踪
python stock_screener.py --strategy zt_follow --top 10   # 涨停板接力

# 多策略组合
python stock_screener.py --strategy oversold,trend --top 5

# 全部策略
python stock_screener.py --strategy all --top 5

# JSON输出
python stock_screener.py --strategy oversold --output json
```
...
| 策略 | 说明 | 核心条件 |
|------|------|----------|
| oversold | 超跌反弹 | 当日跌幅>5%，成交额>5000万 |
| trend | 趋势跟踪 | 当日涨幅2%-8%，成交额>1亿 |
| dividend | 高股息价值 | 成交额>2亿，涨跌幅稳健 |
| growth | 成长股 | 当日涨幅1%-8%，成交额>8000万 |
| northbound | 北向资金 | 成交额>3亿，涨跌幅适中 |
| zt_follow | 涨停接力 | 当日涨幅>8%，成交额>1亿 |

**选股+分析一体化流程：**
1. 先用 `stock_screener.py` 选出候选股票
2. 再用 `stock_full_report.py` 对感兴趣的股票做深度分析
...
2. **执行MCP搜索：** 根据实际需要搜索（通常3-6次）
   ...
   - 券商研报、目标价
   - 最新新闻、政策动态
3. **逐步完成Part I-V分析：** 严格按照五段式分析框架，每个Part达到要求
4. **输出MD报告：** `output/zbs-<股票名称>.md`
...
2. **读取范例并提取参照：** `examples/zbs-中国长城.html` → 用 grep 提取 nav 锚点列表、section id 列表，保存为临时参照文件
3. **读取校验规范：** `HTML手写参考.md` → 严格按「分批机械校验」表格逐批执行
4. **读取数据：** `output/zbs-{股票名称}.md` + `output/data_{代码}.json`
...
**参考范例：** `examples/zbs-中国长城.html`（864行，金标准成品）

> ⚠️ **参考范例路径不可变。** 该文件位于 `examples/` 目录（稳定范例），禁止替换为 `output/` 下的生成结果。行号基于当前版本，如范例更新需同步修正行号。

| 批次 | Part I: Read范例（先看再写） | Part II: Write | Part III: grep兜底 |
|------|---------------------------|---------------|------------------|
...
| Batch 3 | `Read 中国长城 line=281-450`（Part I-III） | Write Part I-III 卡片（mission/macro/chain/quality） | `grep -o 'id="[^"]*"'` → mission/macro/chain/quality 四者存在 |
| Batch 4 | `Read 中国长城 line=451-600`（Part III-IV） | Write Part III-IV 卡片（elasticity/risk/valuation） | `grep -o 'id="[^"]*"'` → elasticity/risk/valuation 三者存在, `grep -c 'score-fill.*width'`≥10（每个fill必须带width）, `grep 'border-top:2px solid var(--gold)'` 弹性树横线存在 |
| Batch 5 | `Read 中国长城 line=601-720`（Part IV-V+footer） | Write Part IV-V 卡片 + footer + `</div>` | `grep -o 'id="[^"]*"'` → compare/tracking 存在, `grep '📌'` 存在, `grep -c '<p>' footer段`=5, `grep 'strong' footer段`=0 |
...
**Part I-V 关键可视化组件清单：**
- **Part I:** 4个信息卡片（标的/周期/风格/命题）+ 核心矛盾高亮框
- **Part II:** 产业链SVG图谱、grid-2布局、关键判断提示框
- **Part III:** grid-2布局、6个评分条、评级徽章 + 弹性树、公式卡片2x2、情景分析grid-3 + 止损信号列表
- **Part IV:** 三档目标价、盈亏比量化卡片 + 对比表格、增长引擎切换3卡片
- **Part V:** 执行清单、verdict-highlight、五维评分、📌总结框（强制，红色左边框）
...
- **禁止 h2/h3 标题带"Part X"编号前缀**：卡片标题只写中文描述（如"任务锁定与核心矛盾"），`.sub` 同样禁止编号
- **Hero 第一项 meta-item** 显示 Part III 公司质地评分的等级（如"B级"），标签写"公司质地"
- **两套评分标签区分**：Part III 评分 → 标签"公司质地评分"；Part V 评分 → 标签"综合交易评分"。两套分维度不同、数值不同是正常的，标签区分清楚即可
...
- **📌总结框为强制组件**：Part V 底部必须有一个独立的红色左边框总结框（`border-left:4px solid var(--red-up)`），包含 `📌 总结` 标题和一段话总结，不可省略或用 verdict-highlight 替代
...
├── zbs-<股票名称>.md          # Phase 2: 完整分析报告
└── zbs-<股票名称>.html        # Phase 3: 可视化HTML报告
```
...
## Part II: 产业链深度拆解（100行）
- 2a. 题材来源判断
- 2b. 产业链图谱（ASCII/Mermaid）
- 2c. 趋势三要素
- 2d. 价值链利润分布

## Part III: 公司筛选与质量评分（50行）
- 3a. 正面筛选清单
- 3b. 不碰清单
- 3c. 质量评分（100分制）

## Part III: 业绩弹性与风险测算（80行）
- 4a. 分业务弹性树
- 4b. 价格敏感度公式
- 4c. 情景分析
...
- `<body>` 结构参考 `examples/zbs-中国长城.html`
...
**2.3 AI逐步分析Part I-V（30-60分钟）**

**⚠️ MD写作禁止事项（防止内容泄露到HTML）：**
- ❌ MD正文任何位置禁止出现：作者署名、社交媒体ID等个人信息
- ❌ MD末尾禁止添加署名行
- ✅ 品牌信息仅出现在HTML的footer中，由Phase 3统一添加
...
□ Part III: 公司筛选与质量评分（50行）
  ...
  ├─ 3a. 正面筛选清单（6项标准）
  ├─ 3b. 不碰清单（负面排查）
  └─ 3c. 公司质地评分（100分制，6个维度——注意：与Part V综合交易评分维度不同，两个分不一定相等）
...
□ Part V: 跟踪计划与综合结论（50行）
  ...
  ├─ 8b. 执行清单（短线/中线）
  ├─ 8c. 综合结论（一句话判断、风险等级、操作建议）
  └─ 8d. 五维综合交易评分（基本面/资金面/技术面/情绪面/事件——与Part III公司质地评分维度不同）

✅ 总计：540行（最低要求）
```

**执行规则：**
1. AI必须严格按照Part I→Part V顺序执行
2. 每完成一个Part，输出：`✅ Part X完成（XX行）`
3. 不能跳过或简化任何Part
4. 每个Part必须达到最低行数要求
...
1. **读取模板：** `shared/template_base.css` + `shared/template_base.js`
2. **读取范例：** `examples/zbs-中国长城.html`
3. **读取数据：** `output/zbs-{股票名称}.md` + `output/data_{代码}.json`
4. **分批手写HTML：**
   - 每批 ≤300行，用 **Write工具** 直接写HTML markup
...
Batch 2: Write — K线图卡片 + Part I-III 卡片                            ~200行
Batch 3: Write — Part III-IV 卡片（弹性/风险/估值）                      ~200行
Batch 4: Write — Part IV-V 卡片 + footer + </div>                      ~150行
...
- **Part III:** grid-2布局、6个评分条、评级徽章 + 弹性树、公式卡片2x2、情景分析grid-3 + 止损信号列表
- **Part IV:** 三档目标价、盈亏比量化卡片 + 对比表格、增长引擎切换3卡片
- **Part V:** 执行清单、verdict-highlight、五维评分、📌总结框
...
- **📌总结框必须为独立 `.card`（红色左边框），位于 Part V 之后、footer 之前**
...
[Phase 2/3] AI分析中...
  ✅ Part I-V 完成，MD已保存：output/zbs-赣锋锂业.md（619行）

[Phase 3/3] AI手写HTML中...
  ✅ 模板已读取（shared/template_base.css + shared/template_base.js）
  ✅ HTML已保存：output/zbs-赣锋锂业.html（991行/63KB）
```
...
### Q1: 为什么Phase 2这么慢？
A: Phase 2需要AI逐步分析Part I-V，每个Part都需要深度思考和MCP搜索，总计需要30-60分钟。这是为了保证分析质量。

### Q2: 可以跳过某些Part吗？
A: 不可以。Part I-V是完整的分析框架，跳过任何一步都会导致分析不完整。
...
- [examples/zbs-中国长城.html](examples/zbs-中国长城.html) — 成品参考范例
- [HTML报告设计规范v1.0.md](HTML报告设计规范v1.0.md) — CSS设计系统详细文档
- [分析框架.md](分析框架.md) — Part I-V 完整分析方法论
