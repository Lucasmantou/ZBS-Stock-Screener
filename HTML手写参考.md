...
| `examples/zbs-中国长城.html` | 成品范例 | 标准参考范例，包含弹性树连线/公式卡片/总结框等全部组件 |
...
<p style="font-size:11px;color:var(--text-muted);">数据来源：akshare(雪球/新浪/同花顺/东方财富/巨潮资讯) · 个股深度研究系统 v2.2</p>
<p style="margin-top:6px;">作者：ZBS</p>
</footer>
```
...
**参考范例：** `examples/zbs-中国长城.html`（864行，已知正确产出）。

### 前置：提取范例参照数据

```bash
# 提取范例的13个导航锚点（参照基准）
grep -o 'href="[^"]*"' examples/zbs-中国长城.html | sort > /tmp/ref_nav.txt

# 提取范例的section id列表
grep -o 'id="[^"]*"' examples/zbs-中国长城.html | grep -v 'chart-\|MathJax\|themeToggle\|arrow-\|svg-' | sort > /tmp/ref_ids.txt
```
...
| 步骤 | 操作 | 校验命令 | 通过条件 |
|------|------|---------|---------|
| 1. Read | `Read examples/zbs-中国长城.html line=1-121` | — | — |
...
| # | 检查项 | 命令 | 通过条件 | Bug 防御 |
|---|--------|------|---------|---------|
| 1 | 导航锚点配对 | `grep -oc 'href="#' zbs-XXX.html` + `grep -o 'id="[^"]*"' \| grep -cE 'hero\|conclusion-top\|...'` | nav数 = 13, 关键id = 13 | 导航死链 |
| 2 | 锚点逐对验证 | `grep -o 'href="[^"]*"' \| tr -d 'href="' \| while read a; do grep -q "id=\\"${a#\#}\\"" || echo "MISSING: ${a#\#}"; done` | 无输出 | 导航死链 |
| 3 | `</html>` 唯一 | `grep -c '</html>' zbs-XXX.html` | = 1 | 结构缺失 |
| 4 | `</script>` 齐全 | `grep -c '</script>' zbs-XXX.html` | ≥ 4（3 CDN + 1 主JS） | **Bug 2: 全页 JS 炸裂** |
| 5 | 主JS闭合在末尾 | `tail -20 zbs-XXX.html \| grep '</script>'` | 存在 | **Bug 2** |
| 6 | 评分条用 `<div>` | `grep -c '<span class="score-fill"' zbs-XXX.html` | = 0 | **Bug 3: 色条不显示** |
| 7 | MathJax 单反斜杠 | `grep -c '\\\\times\|\\\\frac\|\\\\alpha' zbs-XXX.html` | = 0 | **Bug 4: 公式不渲染** |
| 8 | 占位符清零 | `grep -c '__RAW_DATA_ARRAY__\|__PIE_DATA_ARRAY__\|替换为' zbs-XXX.html` | = 0 | **Bug 1: 数据乱码** |
| 9 | 子标题无编号前缀 | `grep -cE '<(h3\|strong)[^>]*>[0-9]+[a-c]\.' zbs-XXX.html` | = 0 | 格式污染 |
| 10 | Footer 结构完整 | `grep -c '免责声明' zbs-XXX.html` + `grep -c '作者：ZBS' zbs-XXX.html` | 全部 ≥ 1 | Footer 丢失 |
