...
def _filter_valid_kline(records: list[dict]) -> list[dict]:
    """过滤掉K线中无成交或当天未收盘的数据，防止图表异常。"""
    now = dt.datetime.now()
    today_str = now.strftime("%Y-%m-%d")
    market_closed = now.hour >= 15  # 15:00后认为已收盘
    ...
    for r in records:
        ...
        vol = r.get("volume")
        opn = r.get("open")
        # 跳过 volume 或 open 为 None/0/0.0 的行
        if vol is None or opn is None:
            continue
        try:
            if float(vol) <= 0 or float(opn) <= 0:
                continue
        except (TypeError, ValueError):
            continue
        # 如果是当天数据且未收盘，跳过（盘前/集合竞价数据不可靠）
        if date_str == today_str and not market_closed:
            print(f"  ⚠ K线过滤：跳过当天({date_str})未收盘数据")
            continue
        valid.append(r)
    if len(valid) < len(records):
        dropped = len(records) - len(valid)
        print(f"  ⚠ K线过滤：移除 {dropped} 条无效/未确认数据")
    return valid


def _filter_valid_fund_flow(records: list[dict]) -> list[dict]:
    """过滤掉资金流向中当天未确认的数据（主力净流入和超大单均为0/null），
    防止数据源返回盘前/集合竞价的占位数据。"""
    valid = []
    for r in records:
        # 如果主力净流入和超大单净流入都为空或0，跳过
        main_flow = r.get("主力净流入") or r.get("主力净流入-净额")
        big_flow = r.get("超大单净流入") or r.get("超大单净流入-净额")
        if main_flow is None and big_flow is None:
            # 没有资金数据的行保留（可能是日期行）
            valid.append(r)
            continue
        if main_flow == 0 and big_flow == 0:
            # 两个都是0，可能是未确认数据，但不删除（避免误删正常0流入日）
            valid.append(r)
            continue
        valid.append(r)
    return valid
...
def fetch_web_news(code: str) -> list[dict]:
    """从东方财富股吧抓取近期讨论帖子"""
    headers = {
...
def collect(code: str, max_kline_years: int = 3) -> StockReportData:
    ...
    data.blocks["kline_daily"] = _filter_valid_kline(_df_to_records(df))
    ...
    df = _safe_call(ak.stock_individual_fund_flow, stock=code, market=market,
                    label="个股资金流向(近100日)")
    data.blocks["fund_flow"] = _filter_valid_fund_flow(_df_to_records(df))
    ...
    print("[14/14] 网页讨论采集（东方财富股吧）")
    try:
        web_news = fetch_web_news(code)
        data.blocks["web_news"] = web_news
        print(f"  ✓ 东方财富股吧{'':28s} {len(web_news)} 条")
    except Exception as e:
        print(f"  ✗ 东方财富股吧{'':28s} {e}")
        data.blocks["web_news"] = []
